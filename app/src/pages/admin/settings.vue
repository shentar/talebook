        ...
        <v-card class="my-2 elevation-4" v-for="card in cards" :key="card.title">
            ...
            <v-card-text v-show="card.show">
                ...
                <template v-if="card.tips">
                    <p v-for="t in card.tips" :key="t.text">{{ t.text }} <a v-if="t.link" target="_blank"
                                                                            :href="t.link">链接</a></p>
                </template>
                ...
                <template v-for="g in card.groups">
                    <v-checkbox small hide-details v-model="settings[g.key]" :key="g.label"
                                :label="g.label"></v-checkbox>
                    <template v-if="settings[g.key]">
                        <template v-for="f in g.fields">
                            <v-textarea outlined v-if="f.type === 'textarea' " :prepend-icon="f.icon"
                                        v-model="settings[f.key]" :key="f.key" :label="f.label"></v-textarea>
                            <v-text-field v-else :prepend-icon="f.icon" v-model="settings[f.key]" :key="f.key"
                                          :label="f.label" type="text"></v-text-field>
                        </template>
                    </template>
                </template>
                ...
                <template v-if="card.show_friends">
                    <v-row v-for="(friend, idx) in settings.FRIENDS" :key="'friend-'+friend.href">
                        <v-col class='py-0' cols=3>
                            <v-text-field flat small hide-details single-line v-model="friend.text" label="名称"
                                          type="text"></v-text-field>
                        </v-col>
                        <v-col class='pa-0' cols=9>
                            <v-text-field flat small hide-details single-line v-model="friend.href" label="链接"
                                          type="text"
                    ...
                    <v-row>
                        <v-col align="center">
                            <v-btn color="primary" @click="settings.FRIENDS.push({text:'', href: ''})">
                                <v-icon>add</v-icon>
                                添加
                            </v-btn>
                        </v-col>
                    ...
                    <v-row v-for="s in settings.SOCIALS" :key="'social-'+s.value">
                        ...
                        <v-col class='py-0' cols=12 sm=7>
                            <v-text-field small hide-details single-line
                                          ...
                                          label="Secret" type="text"></v-text-field>
                        </v-col>
                    </v-row>
                </template>
                ...
                groups: [
                    {
                        ...
                        fields: [
                            {
                                icon: "info",
                                key: "downloads_count_per_ip_limitation",
                                label: "单个IP地址每天可以下载的书籍数量（配置为0表示不限制，默认值为0）"
                            },
                        ],
                    },
                    {
